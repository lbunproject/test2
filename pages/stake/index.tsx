...
import { queryWalletInventory } from 'client/query'
...
const Trade = () => {
  ...
  useEffect(() => {
    ...
    if (peerAddress) {
      ...
      queryWalletInventory(peerAddress).then((inventory) => {
  ...
  useEffect(() => {
    if (wallet) {
      ...
      queryWalletInventory(wallet?.address).then((inventory) => {
  ...
  const totalGas = Math.ceil(stakeMsgs.length / 2) * 1499999;
