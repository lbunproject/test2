  ...
  try {
    ...
    for (let validClaimNft of validClaimNfts) {
      ...
      if (myClaimNftsJson && myClaimNftsJson.data.info) {
        ...
        const collectionAttribute = collectionAttributes.find(attr => attr.nftContract === validClaimNft.token_address);
        ...
        //Info to display
        const earnedRewards = (((Number(collectionAttribute?.reward_amount) * multiplier) / 1000000) * Number(staking_cycles)).toFixed(1)
